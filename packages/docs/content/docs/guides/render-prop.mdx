...
description: "Customize the underlying element with the render prop."
---

Several @aussieljk/frosted components support a `render` prop that allows you to customize the underlying element while preserving the component's functionality and styling. This is useful for integrating with routing libraries, using semantic HTML elements, or composing with other components.
...
<Text render={<label htmlFor="email" />}>Email address</Text>;
```
...
<Text render={(props) => <label {...props} htmlFor="email" />}>Email address</Text>;
```
...
import { Text, Card, Heading } from '@aussieljk/frosted';
...
<Text render={<Link to="/dashboard" />}>Go to Dashboard</Text>;
```
...
import { Card } from '@aussieljk/frosted';

<Card render={<Link to="/users/$userId" params={{ userId: '123' }} />}>User Profile Card</Card>;
```
...
<Link render={<NextLink href="/dashboard" prefetch={false} />}>Go to Dashboard</Link>
```
...
```tsx
import { Text, Heading } from '@aussieljk/frosted';
...
```tsx
import { Text, TextField } from '@aussieljk/frosted';
...
```tsx
import Link from 'next/link';
import { Card, Heading, Text } from '@aussieljk/frosted';
...
```tsx
import { WidgetStack, IconButton } from '@aussieljk/frosted';
...
<WidgetStack.Root>
  ...
  <WidgetStack.Content>{/* slides */}</WidgetStack.Content>
...
<Text render={(props) => <label>Email</label>}>Email</Text>
```
...
<Text render={(props) => <label {...props}>Email</label>}>Email</Text>
```
...
```tsx
<Text render={(props) => <label {...props} htmlFor="email" />}>Email</Text>
```

## Migration from asChild

If you're migrating from an older version of @aussieljk/frosted that used `asChild`, here's how to update:
