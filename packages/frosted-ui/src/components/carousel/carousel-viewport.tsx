...
import { PAGE_SCROLL_FACTOR, getScrollBehavior, getSnapAlignment, useScrollGalleryContext } from './carousel-context';
...
interface CarouselViewportState extends Record<string, unknown> {
  activeIndex: number;
...
interface CarouselViewportProps extends useRender.ComponentProps<'div', CarouselViewportState> {}

const viewportStateAttributesMapping = {
  activeIndex: (value: unknown) => ({ 'data-active-index': String(value) }),
  orientation: () => null,
  canScrollPrev: (value: unknown) => (value ? { 'data-can-scroll-prev': '' } : null),
  canScrollNext: (value: unknown) => (value ? { 'data-can-scroll-next': '' } : null),
  scrolling: (value: unknown) => (value ? { 'data-scrolling': '' } : null),
};
...
const CarouselViewport = React.forwardRef<HTMLDivElement, CarouselViewportProps>(
  function CarouselViewport(props, forwardedRef) {
    ...
    const computeActiveIndex = React.useCallback(() => {
      ...
      const beforeIndices = positions.map((pos, i) => ({ pos, i })).filter(({ pos }) => pos < distributeRange);
      ...
      const afterIndices = positions.map((pos, i) => ({ pos, i })).filter(({ pos }) => pos > endThreshold);
    ...
    const state = React.useMemo<CarouselViewportState>(
    ...
    const scrollBehaviorStyle = scrollBehavior === 'instant' ? { scrollBehavior: 'auto' as const } : undefined;
    ...
    return useRender({
      ...
      props: mergeProps<'div'>(
        {
          className: 'fui-CarouselViewport',
...
export type { CarouselViewportProps, CarouselViewportState };
