...
export default {
  ...
  Error() {
    return <Button onClick={() => toast.error('Something went wrong')}>Show error toast</Button>;
  },
  ...
  Warning() {
    return <Button onClick={() => toast.warning('Your session will expire in 5 minutes')}>Show warning toast</Button>;
  },

  Loading() {
    return <Button onClick={() => toast.loading('Connecting to server...')}>Show loading toast</Button>;
  },

  Default() {
    return <Button onClick={() => toast('A new notification has arrived')}>Show default toast</Button>;
  },
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
        <div style={{ maxWidth: 480 }}>
          <Text size="2" color="gray">
            ...
            <Code size="1">{'<Toast.Root><Toast.Content>…</Toast.Content></Toast.Root>'}</Code> to get the standard
            chrome (animations, swipe-to-dismiss, stacking). Use <Code size="1">Toast.Title</Code> and{' '}
