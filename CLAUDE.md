...
- **Fixtures are colocated**: `src/components/<name>/<name>.fixture.tsx`, one multi-fixture file per component (default-exported object; keys are the variant names). Standalone usage demos live in `packages/frosted-ui/demos/*.fixture.tsx` and import from `@aussieljk/frosted` (aliased to `src/` in `vite.config.ts`) because their source is meant to be copy-pasteable.
- **`cosmos.config.json` runs `lazy: true`** deliberately — eager mode imports every fixture (and therefore the whole library) into the renderer on first load, which is exactly the slow-boot problem the old setup was tuned to avoid. Cost: the sidebar shows a multi-fixture file's variant names only after the file is opened once.
- **Controls come from generated data**: `useComponentControls('<Name>')` (`cosmos/controls.ts`) seeds one cosmos input per prop from `cosmos/generated/component-props.json` (`bun run generate:props`, TS-checker-extracted). A component's `Default` fixture uses it; variant fixtures hardcode their props. The hook loop is safe because the input set is constant per fixture, and `.oxlintrc.json` switches `rules-of-hooks` off for `*.fixture.tsx` for this reason.
- **Theme/accent/gray selects** on every fixture come from the shared decorator (`cosmos/ThemeDecorator.tsx`, wired via the `cosmos.decorator.tsx` files in `src/` and `demos/`), which also mounts the `Toaster`. Global CSS enters through `globalImports` in `cosmos.config.json` → `cosmos/preview.css` (tailwind + built `styles.css` + `theme.css` — the dev script keeps both watched).
...
- **Enumerate fixtures programmatically** with `getFixtures()` from `react-cosmos` (bun runs the TSX imports natively) — that's how `scripts/screenshot-demos.ts` learns the variant names inside multi-fixture files; the dev server's `/cosmos.fixtures.json` only lists files.
...
- **TypeScript 7 (native compiler) is the repo default**, and it has no JS compiler API. `tools/props-gen` exists solely to scope a classic-TS pin (`typescript@6`) for prop-data generation, which needs the compiler API. Don't "clean up" that package or its nested `node_modules/typescript` — `doctor.ts` allowlists it deliberately.
